"""
Generate report/chapters/*.tex from report/ICA_1.docx preserving body order.
Skips Word TOC paragraphs; pairs images with Caption paragraphs; tables from w:tbl.
Run from repo root:  python scripts/ica1_emit_latex.py
"""
...
META = ROOT / "report" / "_ica1_extract"


def tex_escape(s: str) -> str:
    """Escape for LaTeX; newlines become spaces; internal runs of spaces preserved."""
    s = s.replace("\r\n", "\n").replace("\r", "\n")
    s = s.replace("\n", " ")
...
def emit_latex(doc: Document, rid_files: dict[str, Path]) -> str:
    ...
    def copy_embed(rid: str) -> str:
        ...
        if not src:
            return ""
        fig_n += 1
        dest = FIG / f"ICA1_embed_{fig_n}{src.suffix}"
    ...
    while i < len(blocks):
        ...
        if typ == "p":
            ...
            if rids:
                ...
                if i + 1 < len(blocks) and blocks[i + 1][0] == "p":
                    ...
                    if "caption" in nst or nb.text.strip().lower().startswith(
                        "figure"
                    ):
                        cap = tex_escape(nb.text.strip())
                        i += 2
                    else:
                        cap = tex_escape(txt) if txt else ""
                        i += 1
                else:
                    cap = tex_escape(txt) if txt else ""
                    i += 1
            ...
            if (st.lower().startswith("caption") or st == "Caption") and txt.lower().startswith(
                "figure"
            ):
                i += 1
                continue
        ...
        if typ == "t":
            ...
            lines += [
                ...
                "  \\small",
                "  \\caption{}",
                f"  \\label{{tab:ica1_orphan_{tab_n}}}",
                table_tex(b),
                "\\end{table}",
                "",
            ]
...
def split_write(tex: str) -> None:
    ...
    for block in parts:
        ...
        if fname:
            content = (
                f"% Generated from ICA_1.docx — re-run: python scripts/ica1_emit_latex.py\n"
                f"\\chapter{{{title}}}\n\\label{{{lbl}}}\n\n{body}\n"
            )
...
def main() -> None:
    ...
    META.joinpath("ica1_generated.tex").write_text(tex, encoding="utf-8")
    split_write(tex)
    print("OK:", META / "ica1_generated.tex")
