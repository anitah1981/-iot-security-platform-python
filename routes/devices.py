        ...
        if name:
            search_query = {
                "$or": [
                    ...
                    {"deviceId": {"$regex": re.escape(name), "$options": "i"}},
                ]
            }
            existing_scope = filter_query.pop("$or", None)
            if existing_scope:
                filter_query["$and"] = [{"$or": existing_scope}, search_query]
            else:
                filter_query["$and"] = [search_query]
