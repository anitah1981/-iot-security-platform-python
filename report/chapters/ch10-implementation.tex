...
The application also mounts Stripe-backed payment routes (\texttt{/api/payments}) so that, where enabled, users may subscribe to tiered plans (for example \emph{pro} or \emph{business}). This supports sustainable operation---notification channels, device counts, and related limits are tied to \texttt{plan\_limits} middleware---but it is \emph{not} part of the core security-monitoring research question; it is documented as production-oriented scaffolding rather than as an evaluated project outcome.
...
\begin{figure}[htbp]
  \centering
  \includegraphics[width=0.92\textwidth]{figures/ICA2_embed_11.png}
...
The \texttt{services/security\_monitor.py} module concentrates additional IP and change-pattern checks used from heartbeat-related flows, while \texttt{services/incident\_correlator.py} can group related alerts within a configurable time window into incident records for the dashboard---both sit alongside the primary sweep and rule-based pipeline rather than replacing it.
