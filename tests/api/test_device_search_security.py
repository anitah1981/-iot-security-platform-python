    ...
    for key, expected in query.items():
        ...
        if isinstance(expected, dict):
            ...
            if "$regex" in expected:
                ...
                if re.search(expected["$regex"], str(actual or ""), options) is None:
                    return False
...
async def test_device_name_search_preserves_user_scope(monkeypatch):
    ...
    response = await devices_route.get_devices(
        device_type=None,
        status=None,
        name="Kitchen",
        page=1,
...
@pytest.mark.asyncio
async def test_device_name_search_escapes_regex_metacharacters(monkeypatch):
    user_id = ObjectId()
    fake_devices = _FakeCollection(
        [
            _device(user_id, "Kitchen Camera"),
            _device(user_id, "Front Door"),
        ]
    )
    fake_db = type(
        "FakeDb",
